"""
One-batch overfit test for Le-ADMM.
Tests if the model can overfit a single batch (sanity check for implementation).
"""
...
print("=" * 60)
print("One-batch overfit test for Le-ADMM")
print("=" * 60)
...
optimizer = optim.Adam(model.parameters(), lr=1.0)  # Very aggressive
...
for epoch in range(n_epochs):
    ...
    if (epoch + 1) % 20 == 0:
        print(f"Epoch {epoch+1:3d}: loss={loss.item():.6f}, mse={loss_dict['mse_loss'].item():.6f}, lpips={loss_dict['lpips_loss'].item():.6f}")
